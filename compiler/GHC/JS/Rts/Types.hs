...
import           GHC.Types.ForeignCall (Safety(..), CCallConv(..))
...
stackDepth :: GenState -> Int
stackDepth = ggsStackDepth . gsGroup
...
withRegs' start end = withRegs (numReg start) (numReg end)
...
moduleGlobalSymbol :: DynFlags -> Module -> ShortText
moduleGlobalSymbol dflags m
  = "h$" <>
    T.pack (zEncodeString $ showModule dflags m) <>
    "_<global>"

moduleExportsSymbol :: DynFlags -> Module -> ShortText
...
globalIdCache :: GenState -> GlobalIdCache
globalIdCache = ggsGlobalIdCache . gsGroup
...
jsId :: Id -> G JExpr
jsId i = ValExpr . JVar <$> jsIdIdent i Nothing IdPlain
...
declIds  i
  | s == 0    = return mempty
  | s == 1    = decl <$> jsIdI i
  | otherwise = mconcat <$> mapM (fmap decl . jsIdIN i) [1..s]
  where
    s  = typeSize (idType i)
