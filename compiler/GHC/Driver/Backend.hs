...
data Backend = Named !BackendName

  -- Through GHC 9.4, the only back ends are legacy back ends, which
  -- are named.  Since the names exist only to support legacy clients
  -- of the GHC API, future back ends should without names, as
  -- additional value constructors for `Backend`.
...
backendName (Named NCG) = Just NCG
backendName (Named LLVM) = Just LLVM
backendName (Named ViaC) = Just ViaC
backendName (Named Interpreter) = Just Interpreter
backendName (Named NoBackend) = Just NoBackend
