...
endPassIO' :: Logger
           -> LintPassResultConfig
           -> Bool -> PrintUnqualified
           -> CoreToDo -> CoreProgram -> [CoreRule]
           -> IO ()
-- Used by the IO-is CorePrep too
endPassIO' logger lp_cfg dump_core_sizes print_unqual
           pass binds rules
...
data LintConfig = LintConfig
  ...
  , l_vars       :: [Var]            -- ^ Local vars from GHCi
  }

initL :: LintConfig -> LintM a -> WarnsAndErrs    -- Warnings and errors
initL cfg m
...
withoutAnnots pass guts = do
  ...
  withoutFlag $ pass (nukeAnnotsMod guts)
