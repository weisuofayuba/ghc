...
import GHC.Core.Class( classTyVars, classTyCon )
import GHC.Core.Coercion hiding( substCo )
import GHC.Core.Rules
import GHC.Core.Type     hiding ( substTy )
import GHC.Core.TyCon   (TyCon, tyConUnique, tyConName, isNewTyCon )
...
You might think that the Specialiser would have specialised
this, but there are good reasons why it was too early. But
SpecConstr can and should!  It's easy:
...
* Float that dictionary application to top level, thus
  ...
  so the call looks like
    ($wgo 100# @.. lvl)
  Why? The function argToPat will spot contructor applications, but
  not dfun applications.  We could be cleverer, perhaps, and generate
  a RULE like
     $wgo _  @.. ($fMonadStateT @.. @.. $fMonadIdentity = $s$wgo ...
  but it seems simpler just to ensure it floats. See the "Arguments"
  case of Note [Floating to the top] in GHC.Core.Opt.SetLevels.

Wrinkles:
* Only for non-newtype classes!  The newtype ones don't behave
  like data constructors.
...
isValue _env expr       -- Maybe it's a constructor application
  ...
  = case idDetails fun of
        ...
        DFunId is_newtype | not is_newtype -> Just LambdaVal
