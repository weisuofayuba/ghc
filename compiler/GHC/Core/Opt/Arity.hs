...
pedanticBottoms AE{ ae_mode = mode } = case mode of
  BotStrictness                    -> True
  EtaExpandArity{ am_opts = opts } -> arity_pedBot opts
  FindRhsArity{ am_opts = opts }   -> arity_pedBot opts
  ...
  _             -> cheap_dict || cheap_fun e
    where
      cheap_dict = case mb_ty of
                     Nothing -> False
                     Just ty -> (arity_dictsCheap (am_opts mode) && isDictTy ty)
