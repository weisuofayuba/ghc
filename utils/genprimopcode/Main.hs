...
gen_hs_source (Info defaults entries) =
  ...
  where
    ...
    spec ent = ([ "" ] ++) . concat $
      -- Doc comments
      [ case unlatex (escape (desc ent)) ++ extra ent of
          "" -> []
          cmmt -> map ("-- " ++) $ lines $ "|" ++ cmmt
...
{-
Note [The GHC.Prim Story]
~~~~~~~~~~~~~~~~~~~~~~~~~
GHC's primops and primitive types are provided via the very-magical GHC.Prim
module of the ghc-prim package. The bindings of GHC.Prim are all wired-in names
and have special treatment by the compiler and therefore aren't referred to by
generated code.

GHCi is the exception to this rule: Consider, a program like:
    plus :: Int -> Int -> Int
    plus (I# a) (I# b) = a +# b
GHCi's bytecode interpreter needs some way of invoking the +# primop. To do so
it generates a call to the primop's wrapper binding, GHC.Prim.+#. This binding
is defined in the GHC.Prim source module generated by genprimopcode:
    module GHC.Prim where
    ...
    (+#) :: Int# -> Int# -> Int#
    (+#) = \a b -> (+#) a b
    ...
This module is compiled (mostly) like any other Haskell source module and
consequently the occurrence of (+#) in the wrapper's RHS gets the usual
expansion to a machine instruction during code generation.

However, there are a few wrinkles:

 * The GHC.Prim source module won't export bindings for all of the Names. Namely:
   pseudo-ops (e.g. `coerce`) need no wrappers.
 * Haskell does not support levity-polymorphic binders and consequently the wrappers are 
 * We must not generate Typeable bindings for the things defined in GHC.Prim
   since this would induce a module import cycle between GHC.Prim and GHC.Types
   (which defines `Module` and other things used in the Typeable bindings). See
   Note [Grand plan for Typeable] in GHC.Tc.Instance.Typeable.

Note [OPTIONS_GHC in GHC.Prim]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
In GHC.Prim we set some crucial GHC options

* Eta reduction: -fno-do-eta-reduction
  In GHC.Prim we builds a wrapper for each primop, thus
  ...
  `plusInt#`.  When we compile GHC.Prim, the code generator spots
  (plusInt# a b) and generates an add instruction.

  It's very important that we don't eta-reduce this to
      plusInt# = plusInt#
...
* We cannot generated Typeable bindings for primitive types without
  inducing a module import cycle between GHC.Prim and GHC.Types. Consequently,
  we rather inject these bindings into GHC.Types and use -dno-typeable-binds to
  suppress them when compiling GHC.Prim.
-}
